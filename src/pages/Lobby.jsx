...
const ReflexGame = () => {
  ...
  let buttonClass =
    "w-full h-full rounded-2xl shadow-xl transition-all duration-100 ease-in-out";
  let message;
  ...
  switch (gameState) {
    ...
    case "catch":
      buttonClass +=
        " bg-green-500 hover:bg-green-400 transform scale-105 active:scale-100";
    ...
    <div className="mt-8 p-4 bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg mx-auto text-center border-b-4 border-l-4 border-yellow-500">
      ...
      {(gameState === "wait" || gameState === "catch") && (
        <p className="mt-2 text-sm text-gray-300">
          {gameState === "wait"
            ? "Préparez-vous à cliquer..."
            : "Temps de réaction actuel : En cours..."}
        </p>
      )}
    </div>
  );
            ...
            {players.map((p) => (
              ...
              >
                <span className="truncate">
                  {p.pseudo}
                  {p.is_admin && (
                    <span className="ml-2 text-red-400 font-extrabold">(ADMIN)</span>
                  )}
                  {p.id === currentSocketId && (
                    <span className="ml-1 text-xs text-yellow-500">(Moi)</span>
                  )}
                </span>
